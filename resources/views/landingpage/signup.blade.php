                ...
                <div class="md:flex justify-end">
                    <div class="lg:w-1/3 md:w-2/4">
                        <div class="rounded shadow bg-white dark:bg-slate-900 p-6">
                            ...
                            <h5 class="mt-6 text-xl font-semibold">{{ __('landing.auth_register_title') }}</h5>
                            ...
                            <form class="text-start mt-4" method="POST" action="{{ route('register') }}">
                                @csrf
                                <div class="grid grid-cols-1">
                                    <div class="mb-4">
                                        <label class="font-semibold" for="RegisterName">{{ __('landing.auth_name') }}:</label>
                                        <input id="RegisterName" name="name" type="text" class="form-input mt-3 w-full py-2 px-3 h-10 bg-transparent dark:bg-slate-900 dark:text-slate-200 rounded outline-none border border-gray-200 focus:border-amber-400 dark:border-gray-800 dark:focus:border-amber-400 focus:ring-0 @error('name') border-red-500 @enderror" placeholder="{{ __('landing.auth_name') }}" value="{{ old('name') }}" required>
                                        @error('name')
                                    ...
                                    <div class="mb-4">
                                        <label class="font-semibold" for="LoginPasswordConfirmation">{{ __('landing.auth_confirm_password') }}:</label>
                                        <input id="LoginPasswordConfirmation" name="password_confirmation" type="password" class="form-input mt-3 w-full py-2 px-3 h-10 bg-transparent dark:bg-slate-900 dark:text-slate-200 rounded outline-none border border-gray-200 focus:border-amber-400 dark:border-gray-800 dark:focus:border-amber-400 focus:ring-0" placeholder="{{ __('landing.auth_confirm_password') }}" required>
                                    </div>
    
                                    <div class="mb-4">
                                        <div class="flex items-center w-full mb-0">
                                            <input class="form-checkbox rounded border-gray-200 dark:border-gray-800 text-amber-400 focus:border-amber-300 focus:ring focus:ring-offset-0 focus:ring-amber-400 focus:ring-opacity-50 me-2" type="checkbox" value="1" id="AcceptT&C" name="terms" required>
                                            <label class="form-check-label text-slate-400" for="AcceptT&C">{{ __('landing.auth_agree_terms') }} <a href="{{ route('terms') }}" class="text-amber-400">{{ __('landing.auth_terms') }}</a></label>
                                        </div>
                                        @error('terms')
                                            <span class="text-red-500 text-sm mt-1">{{ $message }}</span>
                                        @enderror
                                    ...
                                    <div class="mb-4">
                                        <button type="submit" class="py-2 px-5 inline-block tracking-wide border align-middle duration-500 text-base text-center bg-amber-400 hover:bg-amber-500 border-amber-400 hover:border-amber-500 text-white rounded-md w-full">{{ __('landing.auth_sign_up_button') }}</button>
                                    </div>
                                    ...
                                    <div class="text-center">
                                        <span class="text-slate-400 me-2">{{ __('landing.auth_have_account') }}</span> <a href="{{ route('login') }}" class="text-slate-900 dark:text-white font-bold inline-block">{{ __('landing.auth_sign_in_link') }}</a>
                                    </div>
                                </div>
                            </form>
                        </div>
